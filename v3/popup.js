...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    let currentTab;
    let existingAliasKey = null; // Stores the alias key if the current URL is already saved
    let existingAliasData = null; // Stores the full data of the existing alias
    ...
    const updateUIForNewAlias = () => {
        ...
        aliasInput.value = ''; // Ensure alias is empty for new entry
        descriptionInput.value = currentTab.title || ''; // Pre-fill with title
        keywordsInput.value = '';
    ...
    try {
        ...
        // Check if current URL is already saved under any alias
    ...
    // Event listener for AI generation button
    generateAiButton.addEventListener('click', async () => {
        ...
        statusMessage.className = 'status-message info';
        generateAiButton.disabled = true; // Disable button during generation
        ...
        try {
            ...
            if (response && response.success) {
                ...
                aliasInput.value = response.suggestedAlias; // ALWAYS fill alias with AI suggestion on click
                ...
                keywordsInput.value = '';
                aliasInput.value = ''; // Clear alias if AI fails
            ...
            generateAiButton.disabled = false; // Re-enable button
        }
    });
    ...
    manageButton.addEventListener('click', async () => { // Make async to await message response
